...
keyboard = [
  ...
  [
    Button.url("المطور", "https://t.me/C6ICC")
    ]
]
...
async def op(event):
  ...
  if not event.is_private:
    ...
    legendbye = [
      [
        Button.url("اضغط هنا", f"https://t.me/U_2_6")
        ]
      ]
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      try:
        ...
        await event.respond("ارسل المشكله الى لحلها- @C6ICC\n**LOGS**\n" + str(e))
...
async def start(event):
    keyboard = [
      ...
      [
        Button.url("المالك", "https://t.me/C6ICC")
        ]
    ]
